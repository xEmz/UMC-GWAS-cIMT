...
# SLURM settings
#SBATCH --job-name=plink2_make_bgen_filter    # Job name
...
#SBATCH --output=plink2_make_bgen_filter.log   # Standard output and error log
...
mkdir ${REGENIE_SMART}/BGEN_input_filtered
OUTPUT="${REGENIE_SMART}/BGEN_input_filtered/chr_${SLURM_ARRAY_TASK_ID}.8bit"

#Create function to convert .gen and .sample files to .bgen files for each chromosome with set quality parameters


bgen_generate () {
${PLINK2} --parallel 1 23 --gen ${GEN} ref-first --sample ${SAMPLE} --keep ${NA} --geno 0.01  --hwe 0.001 --maf 0.01 --export bgen-1.2 'bits=8' --out ${OUTPUT}
}
