...
# SLURM settings
#SBATCH --job-name=comparison_plotter_Z    # Job name
...
#SBATCH --output=comparison_plotter_Z.log   # Standard output and error log
...
RegenieResults <- data.frame(ID=RegenieResults$ID, BETAR=RegenieResults$BETA, SER=RegenieResults$SE)

RegenieResults$ZR <- RegenieResults$BETAR/RegenieResults$SER
...
SNPTestResults <- data.frame(ID=SNPTestResults$RSID, BETAS=SNPTestResults$BETA, SES=SNPTestResults$SE)

SNPTestResults$ZS <- SNPTestResults$BETAS/SNPTestResults$SES
...
png(paste0(OUTPUT_loc, "Z_comparison_QC.png"), width = 800, height = 800)
# plot (random sample of) data 
plot(Results$ZS, Results$ZR, 
     xlab = "Calculated Z-score with SNPTest", ylab = "Calculated Z-score with REGENIE", 
     main = "Correlation of Z-scores calculated from SNPTest and REGENIE results", col = "#1290D9", bty="n", pch = 1,
     xaxs="i", yaxs="i")
